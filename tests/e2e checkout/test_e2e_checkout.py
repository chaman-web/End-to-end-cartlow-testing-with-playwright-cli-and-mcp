...
def switch_to_ksa(page: Page):
    """Click UAE dropdown and select KSA channel."""
    page.locator("button:has-text('UAE')").first.click()
    page.wait_for_timeout(1000)
    page.locator("span.cursor-pointer:has-text('KSA')").first.click()
    page.wait_for_timeout(5000)
    # KSA switch is client-side — verify by checking SAR currency or KSA in page
    body = page.locator("body").inner_text()
    assert "SAR" in body or "KSA" in body, "KSA channel switch failed"
    print(f"✅ Switched to KSA channel — {page.url}")
...
def test_e2e_checkout(page: Page):
    ...
    assert any(k in body for k in ["remove", "checkout", "aed", "sar"])
    print("✅ Cart verified")
