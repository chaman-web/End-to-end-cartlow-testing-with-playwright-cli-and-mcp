...
def login_and_switch_ksa(page: Page):
    ...
    # Switch to KSA channel
    page.locator("button:has-text('UAE')").first.click()
    page.wait_for_timeout(1000)
    page.locator("span.cursor-pointer:has-text('KSA')").first.click()
    page.wait_for_timeout(5000)
    assert "KSA" in page.locator("body").inner_text()
    print("✅ Switched to KSA")


def ensure_ksa_cart_has_item(page: Page):
    page.goto(f"{KSA_URL}/checkout/cart", wait_until="domcontentloaded")
    page.wait_for_timeout(5000)
    has_items = page.locator("a[href*='checkout/onepage'], a:has-text('Checkout'), button:has-text('Checkout')").count() > 0
    if has_items:
        print("  Cart has items ✅"); return
    print("  Cart empty — adding product...")
    page.goto(f"{KSA_URL}/search?query={SEARCH}", wait_until="domcontentloaded")
    page.wait_for_timeout(10000)
...
def go_to_ksa_checkout(page: Page):
    page.goto(f"{KSA_URL}/checkout/cart", wait_until="domcontentloaded")
    page.wait_for_timeout(5000)
...
def detect_payment_methods(page: Page) -> list[dict]:
    go_to_ksa_checkout(page)
    print(f"  Checkout URL: {page.url}")
    known_ids = ["paymob", "noonpay", "tamara", "tabby", "coinpayments", "cashondelivery", "mada", "stcpay", "stripe"]
    methods = []
...
def test_payment_methods_ksa(page: Page):
    """Dynamically detect and test each payment method on the KSA checkout page."""
    page.set_viewport_size({"width": 1280, "height": 800})

    login_and_switch_ksa(page)
    ensure_ksa_cart_has_item(page)
    methods = detect_payment_methods(page)
    assert methods, "No payment methods found on KSA checkout page"
    ...
    for method in methods:
        ...
        try:
            ...
            if "paymob" in gateway_url.lower():
                ...
                assert "tamara" in gateway_url.lower()
    ...
    # Summary
    print(f"\n{'='*50}")
    print("📊 KSA Payment Method Test Results:")
